...
| `023_group_join_approval.sql` | groepslid-goedkeuring (GROUP_APPROVAL_PLAN.md): `require_approval`-toggle op `groups` + status `'pending'` toegestaan in `group_members` (joiner wacht op goedkeuring door de owner). Reverse: `023_..._down.sql` (verwijdert pending-rijen) | ja, idempotent |
| `024_exams.sql` | examens (EXAM_PLAN.md): `exams` (persoonlijk of per groep, `exam_date timestamptz`, HARD delete — sync levert een volledige snapshot) + `exam_decks` (koppeltabel, deck mag in meerdere examens, `ON DELETE CASCADE` t.b.v. de tombstone-purge) + `user_card_progress.longest_in_streak_hours` (`INTEGER NULL`, client-berekende langste verdiende gap in de huidige streak, voedt examen-readiness) + trigger + GRANT. Vereist 017/020. **Vereist vóór deploy van de examens-backend.** Reverse: `024_..._down.sql` | ja, idempotent |
